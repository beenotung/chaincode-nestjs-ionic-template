...
const client = Fabric_Client.newDefaultKeyValueStore({
  path: store_path
}).then((state_store) => {
...
async function queryFunction(contractName, functionName, args) {
  await client;
...
async function executeFunction(contractName, functionName, args) {
  await client;
...
function voteUser(username) {
  return executeFunction('vote', 'voteUser', [username])
}

function getUserVote() {
  return queryFunction('vote', 'getUserVote', [''])
}
...
async function invokeFunction_test() {
  console.log('invokeFunction_test begin');
  await log(getUserVote());
  await log(voteUser('beeno'));
  await log(voteUser('YX'));
  await log(voteUser('beeno'));
  await log(getUserVote());
  console.log('invokeFunction_test end')
}
